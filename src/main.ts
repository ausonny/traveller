...
var textToDisplaygamesave: DisplayItem[];
var textToDisplayachievement: DisplayItem[];
var textToDisplayloot: DisplayItem[];
var textToDisplaychallenge: DisplayItem[];
var textToDisplaystory: DisplayItem[];
var displayindex: number;

var initted = false;
var gameData: saveGameData;
var internalInflationArray: number[];
internalInflationArray = [];

var lastachievementcount: number;
lastachievementcount = 0;
var achievementbonusarray: number[];
...
function processStuff(ticks: number) {
  ...
  var autoprestige1 = <HTMLInputElement>document.getElementById("autoprestige1");
...
function updateGUI() {
  ...
  var btndown = document.getElementById("btntierdown") as HTMLButtonElement;
  btndown.disabled = false;
  var btnup = document.getElementById("btntierup") as HTMLButtonElement;
...
function DrawSolidEnemy(ctx: CanvasRenderingContext2D, CurrentHitPoints: JBDecimal, MaxHitPoints: JBDecimal, squareSize: number, enemysize: number, x: number, y: number, enemycolor: string) {
...
function DrawHalfColorEnemy(ctx: CanvasRenderingContext2D, CurrentHitPoints: JBDecimal, MaxHitPoints: JBDecimal, squareSize: number, enemysize: number, x: number, y: number, leftcolor: string, rightcolor: string) {
  ctx.globalAlpha = CurrentHitPoints.divide(MaxHitPoints).ToNumber();
  if (ctx.globalAlpha < 0.1) {
    ctx.globalAlpha = 0.1;
  }
